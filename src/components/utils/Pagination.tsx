...
const Pagination: FC<PaginationControlsProps> = ({
	...
	const getButtonClass = (i: number) =>
		`w-8 h-8 rounded-lg text-sm ${
			i === page
				? "bg-orange-primary cursor-not-allowed text-white"
			...
			<div className="flex justify-between items-center">
				...
				<div className="flex-none p-2">
					<button
						...
						className="flex justify-between w-fit gap-2 items-center  p-2 border border-text-secondary/20 rounded-lg text-sm text-text-primary"
					>
						{per_page}
						<Image
							src={`/report/${
								showDropdown ? "up-arrow.svg" : "down-arrow.svg"
							}`}
							width={100}
							height={100}
							alt={`${showDropdown ? "up" : "down"}-arrow`}
							className="w-2 h-2"
						/>
					</button>
					<div
						...
						} w-fit mt-1 bg-white rounded-lg shadow-xl absolute`}
					>
						{[10, 15, 20].map((perPageOption) => (
							<button
								key={perPageOption}
								className="block px-4 py-2 text-sm text-text-primary hover:bg-gray-100 cursor-pointer"
